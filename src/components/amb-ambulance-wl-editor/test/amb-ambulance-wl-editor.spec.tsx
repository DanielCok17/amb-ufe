 ...
 afterEach(() => { mock.reset(); });
  it('buttons shall be of different type', async () => {
    ...
    const page = await newSpecPage({
      ...
      html: `<amb-ambulance-wl-editor entry-id="test-entry"
      ambulance-id="test-ambulance" api-base="http://sample.test/api"></amb-ambulance-wl-editor>`,
    });
    await delay(300);
  ...
  it('first text field is patient name', async () => {
    ...
    const page = await newSpecPage({
       components: [AmbAmbulanceWlEditor],
       html: `<amb-ambulance-wl-editor entry-id="test-entry" ambulance-id="test-ambulance" api-base="http://sample.test/api"></amb-ambulance-wl-editor>`,
    });
